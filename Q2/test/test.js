...
describe("HelloWorld", function () {
  ...
  it("Should return true for correct proof", async function () {
    ...
    // add input & wait for zk prf on the input. It also generated wasm & circuit key
    ...
    // convert publicSignal from string to Big Int
    const editedPublicSignals = unstringifyBigInts(publicSignals);
    // convert proof from string to Big Int
    const editedProof = unstringifyBigInts(proof);
    //export prf & publicsignal to make solidity understand it, hence creating callData for solidity
    const calldata = await groth16.exportSolidityCallData(
      editedProof,
      editedPublicSignals
    );
    // console.log("calldata", calldata);
    // parse into array-like
    const argv = calldata
      .replace(/["[\]\s]/g, "")
      .split(",")
