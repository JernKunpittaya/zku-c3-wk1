...
template sudoku() {
    ...
    component puzzle_rp[9][9];
    component solution_rp[9][9];
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            puzzle_rp[i][j] = RangeProof(5);
            puzzle_rp[i][j].in <== puzzle[i][j];
            puzzle_rp[i][j].range[0] <== 0;
            puzzle_rp[i][j].range[1] <== 9;
            puzzle_rp[i][j].out === 1;

            solution_rp[i][j] = RangeProof(5);
            solution_rp[i][j].in <== solution[i][j];
            solution_rp[i][j].range[0] <== 0;
            solution_rp[i][j].range[1] <== 9;
            solution_rp[i][j].out === 1;
